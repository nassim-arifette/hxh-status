...
.EXAMPLE
  .\create-issues.ps1 -DryRun     # print without creating anything
  .\create-issues.ps1             # create for real
#>
[CmdletBinding()]
param(
    [Alias("r")]
    [string]$Repo = $(if ($env:REPO) { $env:REPO } else { "nassim-arifette/hxh-status" }),
...
function Invoke-CreateLabel {
    ...
    if ($DryRun) {
        Write-Host "[dry-run] gh label create `"$Name`" --repo `"$Repo`" --color `"$Color`" --description `"$Description`" --force"
    } else {
...
function Invoke-CreateMilestone {
    ...
    if ($DryRun) {
        Write-Host "[dry-run] gh api repos/$Repo/milestones -f title=`"$Title`" --silent"
    } else {
        $output = & gh api "repos/$Repo/milestones" -f "title=$Title" --silent 2>&1
        if ($LASTEXITCODE -ne 0) {
            Write-Host "  milestone `"$Title`" already exists"
        }
    }
}
...
function Invoke-CreateIssue {
    ...
    if ($DryRun) {
        Write-Host "[dry-run] gh issue create --repo `"$Repo`" --title `"$Title`" --label `"$Labels`" --milestone `"$Milestone`" --body `"$Body`""
    } else {
        $tempFile = [System.IO.Path]::GetTempFileName()
        try {
            $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
            [System.IO.File]::WriteAllText($tempFile, $Body, $utf8NoBom)
            & gh issue create --repo $Repo --title $Title --label $Labels --milestone $Milestone --body-file $tempFile
            if ($LASTEXITCODE -ne 0) {
...
Write-Host "== Labels =="
Invoke-CreateLabel -Name 'bug' -Color 'd73a4a' -Description 'Something is broken or behaves incorrectly'
Invoke-CreateLabel -Name 'feature' -Color 'a2eeef' -Description 'New functionality'
Invoke-CreateLabel -Name 'chore' -Color 'cfd3d7' -Description 'Maintenance, tooling, repo hygiene'
Invoke-CreateLabel -Name 'refactor' -Color 'fbca04' -Description 'Restructuring with no behaviour change'
Invoke-CreateLabel -Name 'ci' -Color '0e8a16' -Description 'Continuous integration and GitHub Actions workflows'
Invoke-CreateLabel -Name 'a11y' -Color '7057ff' -Description 'Accessibility'
Invoke-CreateLabel -Name 'i18n' -Color '1d76db' -Description 'Internationalisation and translations'
Invoke-CreateLabel -Name 'performance' -Color 'f9d0c4' -Description 'Bundle size, load time'
Invoke-CreateLabel -Name 'security' -Color 'b60205' -Description 'Security, CSP, secrets'
Invoke-CreateLabel -Name 'seo' -Color '5319e7' -Description 'Search indexing and metadata'
Invoke-CreateLabel -Name 'pwa' -Color '006b75' -Description 'Service worker, manifest, push notifications'
Invoke-CreateLabel -Name 'types' -Color 'bfd4f2' -Description 'TypeScript typing'
Invoke-CreateLabel -Name 'test' -Color 'c2e0c6' -Description 'Test coverage'
Invoke-CreateLabel -Name 'documentation' -Color '0075ca' -Description 'Docs and contributor guides'
Invoke-CreateLabel -Name 'automation' -Color 'e4e669' -Description 'Togashi / publication automation pipeline'
Invoke-CreateLabel -Name 'good first issue' -Color '7057ff' -Description 'Good entry point for a first contribution'
Invoke-CreateLabel -Name 'priority:high' -Color 'b60205' -Description 'Do this first'
Invoke-CreateLabel -Name 'priority:medium' -Color 'fbca04' -Description 'Schedule it'
Invoke-CreateLabel -Name 'priority:low' -Color 'cfd3d7' -Description 'Once everything else is done'

Write-Host "== Milestones =="
Invoke-CreateMilestone -Title 'Quality and reliability'
Invoke-CreateMilestone -Title 'Performance and accessibility'
Invoke-CreateMilestone -Title 'New features'

Write-Host "== Issues =="

Write-Host '  -> #1 build(lint): replace the hand-written file list with `eslint .`'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'build(lint): replace the hand-written file list with `eslint .`' -Labels 'chore,ci,priority:high,good first issue' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #2 ci: add a verification workflow for pull requests'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'ci: add a verification workflow for pull requests' -Labels 'ci,priority:high' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #3 i18n(data): chapter titles are untranslated in pt, zh and ar (31/421)'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'i18n(data): chapter titles are untranslated in pt, zh and ar (31/421)' -Labels 'i18n,bug,priority:high' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #4 a11y: the accessible name on `.chapter-grid` is never exposed'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'a11y: the accessible name on `.chapter-grid` is never exposed' -Labels 'a11y,bug,good first issue' -Milestone 'Performance and accessibility' -Body $BODY

Write-Host '  -> #5 pwa: add a web manifest and an apple-touch-icon'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'pwa: add a web manifest and an apple-touch-icon' -Labels 'pwa,bug,priority:high' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #6 perf: only ship the active locale''s data to the client'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'perf: only ship the active locale''s data to the client' -Labels 'performance,i18n' -Milestone 'Performance and accessibility' -Body $BODY

Write-Host '  -> #7 perf(i18n): replace the client-side locale redirect'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'perf(i18n): replace the client-side locale redirect' -Labels 'performance,i18n,seo' -Milestone 'Performance and accessibility' -Body $BODY

Write-Host '  -> #8 security(csp): drop `unsafe-inline` by consolidating the `LocalDate` scripts'
$BODY = @'
## Context
...
**Estimated effort:** M (~4 h)
'@
Invoke-CreateIssue -Title 'security(csp): drop `unsafe-inline` by consolidating the `LocalDate` scripts' -Labels 'security,performance' -Milestone 'Performance and accessibility' -Body $BODY

Write-Host '  -> #9 refactor(app): collapse the locale pages into `app/[locale]`'
$BODY = @'
## Context
...
**Estimated effort:** S (~2 h)
'@
Invoke-CreateIssue -Title 'refactor(app): collapse the locale pages into `app/[locale]`' -Labels 'refactor,i18n,good first issue' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #10 ci(publication): replace the date-pinned cron with a periodic one'
$BODY = @'
## Context
...
**Estimated effort:** XS (~30 min)
'@
Invoke-CreateIssue -Title 'ci(publication): replace the date-pinned cron with a periodic one' -Labels 'ci,automation,chore' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #11 types: type-check the `worker/` directory'
$BODY = @'
## Context
...
**Estimated effort:** M (~1 day)
'@
Invoke-CreateIssue -Title 'types: type-check the `worker/` directory' -Labels 'types,chore' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #12 seo: disallow `/capture/*` in robots.txt'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'seo: disallow `/capture/*` in robots.txt' -Labels 'seo,good first issue' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #13 repo: add CONTRIBUTING, dependabot and issue/PR templates'
$BODY = @'
## Context
...
**Estimated effort:** S (~2 h)
'@
Invoke-CreateIssue -Title 'repo: add CONTRIBUTING, dependabot and issue/PR templates' -Labels 'documentation,chore,good first issue' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #14 test: no coverage on the React components'
$BODY = @'
## Context
...
**Estimated effort:** M (~1 day)
'@
Invoke-CreateIssue -Title 'test: no coverage on the React components' -Labels 'test' -Milestone 'Quality and reliability' -Body $BODY

Write-Host '  -> #15 feat(api): RSS/Atom feeds and an iCal calendar per locale'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'feat(api): RSS/Atom feeds and an iCal calendar per locale' -Labels 'feature,priority:high' -Milestone 'New features' -Body $BODY

Write-Host '  -> #16 feat(share): embeddable SVG status badge'
$BODY = @'
## Context
...
**Estimated effort:** S (~4 h)
'@
Invoke-CreateIssue -Title 'feat(share): embeddable SVG status badge' -Labels 'feature,priority:high' -Milestone 'New features' -Body $BODY

Write-Host '  -> #17 feat(content): Togashi post archive page'
$BODY = @'
## Context
...
**Estimated effort:** M (~1 day)
'@
Invoke-CreateIssue -Title 'feat(content): Togashi post archive page' -Labels 'feature,seo' -Milestone 'New features' -Body $BODY

Write-Host '  -> #18 feat(tracker): production changelog'
$BODY = @'
## Context
...
**Estimated effort:** M (~1 day)
'@
Invoke-CreateIssue -Title 'feat(tracker): production changelog' -Labels 'feature' -Milestone 'New features' -Body $BODY

Write-Host '  -> #19 feat(stats): hiatus and publication-pace statistics'
$BODY = @'
## Context
...
**Estimated effort:** M (~1 day)
'@
Invoke-CreateIssue -Title 'feat(stats): hiatus and publication-pace statistics' -Labels 'feature' -Milestone 'New features' -Body $BODY

Write-Host '  -> #20 feat(push): granular notification preferences'
$BODY = @'
## Context
...
**Estimated effort:** M (~1 day)
'@
Invoke-CreateIssue -Title 'feat(push): granular notification preferences' -Labels 'feature,pwa' -Milestone 'New features' -Body $BODY

Write-Host '  -> #21 feat(tracker): volume view'
$BODY = @'
## Context
...
**Estimated effort:** S (~4 h)
'@
Invoke-CreateIssue -Title 'feat(tracker): volume view' -Labels 'feature' -Milestone 'New features' -Body $BODY

Write-Host '  -> #22 feat(tracker): current arc progress'
$BODY = @'
## Context
...
**Estimated effort:** S (~4 h)
'@
Invoke-CreateIssue -Title 'feat(tracker): current arc progress' -Labels 'feature,priority:low' -Milestone 'New features' -Body $BODY

Write-Host '  -> #23 feat(ui): light theme'
$BODY = @'
## Context
...
**Estimated effort:** M (~1 day)
'@
Invoke-CreateIssue -Title 'feat(ui): light theme' -Labels 'feature,priority:low' -Milestone 'New features' -Body $BODY

Write-Host '  -> #24 feat(api): outbound webhooks for integrators'
$BODY = @'
## Context
...
'@
Invoke-CreateIssue -Title 'feat(api): outbound webhooks for integrators' -Labels 'feature,priority:low' -Milestone 'New features' -Body $BODY
